...
final class GeneratorTests: XCTestCase {
    ...
    func testSinWave() throws {
        ...
        let mean = Int(vDSP.mean(sig.ts.vector(at: 0)) * 100)
        XCTAssertEqual(mean, 0)
        
        let max = vDSP.maximum(sig.ts.vector(at: 0))
        XCTAssertLessThanOrEqual(max, 1.0)
        
        let min = vDSP.minimum(sig.ts.vector(at: 0))
...
//        noise.next(1000)
//        signal.ts.apply(colIdx: 0, vec: noise.ts.vector(at: 0), op: .add)
//
